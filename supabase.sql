...
begin
  if length(el_put.k) < 20 or length(el_put.k) > 64 then
    raise exception 'bad key';
  end if;
  if pg_column_size(el_put.d) > 4194304 then
    raise exception 'too large';
  end if;
  if not exists (select 1 from public.el_sync s where s.k = el_put.k)
